...
class UrlLibAivisTransport:
    ...
    def post(
        ...
        return self._request(
            "POST", path, query, body if body is not None else b"", content_type
        )
    ...
    def _request(
        ...
        try:
            ...
            raise AivisSpeechError(
                f"AivisSpeech returned HTTP {exc.code} for {method} {path}: {detail}"
            ) from exc
        except error.URLError as exc:
            raise AivisSpeechError(
                f"Could not connect to AivisSpeech at {self.base_url}: {exc.reason}"
            ) from exc


class AivisSpeechClient:
    def __init__(
        self, base_url: str | None = None, transport: AivisTransport | None = None
    ) -> None:
    ...
    def synthesize_to_file(
        ...
        _, query_bytes = self.transport.post(
            "/audio_query", {"text": text, "speaker": str(speaker_id)}, None, None
        )
        ...
        _, wav_bytes = self.transport.post(
            "/synthesis", {"speaker": str(speaker_id)}, body, "application/json"
        )
    ...
    def _resolve_speaker_id(self, speaker: str | int) -> int:
        ...
        for entry in self._speakers():
            ...
            if entry.get("name") == speaker_text:
                talk_styles = [
                    style for style in styles if style.get("type", "talk") == "talk"
                ]
                selected = (talk_styles or styles)[0] if styles else None
                if selected and selected.get("id") is not None:
                    return int(selected["id"])
        raise ValueError(
            f"AivisSpeech speaker must be a numeric style ID or a name from /speakers: {speaker_text}"
        )
...
def _resolve_base_url(base_url: str | None) -> str:
    return (
        base_url or os.environ.get("AIVIS_SPEECH_BASE_URL") or "http://127.0.0.1:10101"
    )
