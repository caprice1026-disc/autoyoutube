...
if [ "$#" -lt 1 ]; then
  echo "Usage: scripts/make-video-with-generated-intro.sh <project.youtube.json> [args...]" >&2
  exit 40
fi
...
if [ "$needs_aivis" -eq 1 ]; then
  if ! curl -fsS "$AIVIS_BASE_URL/version" >/dev/null 2>&1; then
    echo "[make-video-with-generated-intro.sh] AivisSpeech is not ready; starting Docker service."
...
PYTHONUTF8=1 PYTHONIOENCODING=utf-8 \
  "$PYTHON_BIN" -m src.generated_intro_main "$PROJECT_PATH" "$@"
