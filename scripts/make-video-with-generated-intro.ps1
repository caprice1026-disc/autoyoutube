...
if ($VoiceMode -eq "aivis" -and -not $DryRun -and -not $PlanOnly) {
  if (-not (Test-AivisReady -BaseUrl $AivisBaseUrl)) {
    Write-Host "[make-video-with-generated-intro.ps1] AivisSpeech is not ready; starting Docker service."
...
$argsList = @("-m", "src.generated_intro_main", $ProjectPath)
if ($GeneratedIntroPath) { $argsList += @("--generated-intro-path", $GeneratedIntroPath) }
foreach ($keyword in $VisualKeyword) { $argsList += @("--visual-keyword", $keyword) }
...
switch ($code) {
  0 { Write-Host "[make-video-with-generated-intro.ps1] succeeded." }
  10 { Write-Host "[make-video-with-generated-intro.ps1] succeeded with warnings." }
  20 { Write-Error "[make-video-with-generated-intro.ps1] auto-fix limit reached." }
  30 { Write-Error "[make-video-with-generated-intro.ps1] non-fixable quality error." }
  40 { Write-Error "[make-video-with-generated-intro.ps1] environment error." }
  50 { Write-Error "[make-video-with-generated-intro.ps1] external API error." }
  60 { Write-Error "[make-video-with-generated-intro.ps1] render error." }
  70 { Write-Error "[make-video-with-generated-intro.ps1] encoding error." }
  80 { Write-Error "[make-video-with-generated-intro.ps1] upload blocked." }
  default { Write-Error "[make-video-with-generated-intro.ps1] unknown failure: $code" }
}
exit $code
